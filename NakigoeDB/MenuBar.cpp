...
MenuBar::MenuBar(const Array<std::pair<String, Array<String>>>& menus)
{
...
Optional<MenuBarItemIndex> MenuBar::update()
{
...
MenuBar& MenuBar::setItemEnabled(const size_t menuIndex, const size_t itemIndex, const bool enabled)
{
	return setItemEnabled(MenuBarItemIndex{ menuIndex, itemIndex }, enabled);
}

MenuBar& MenuBar::setItemEnabled(const MenuBarItemIndex& itemIndex, const bool enabled)
{
...
bool MenuBar::getItemEnabled(const size_t menuIndex, const size_t itemIndex) const
{
	return getItemEnabled(MenuBarItemIndex{ menuIndex, itemIndex });
}

bool MenuBar::getItemEnabled(const MenuBarItemIndex& itemIndex) const
{
	assert(itemIndex.menuIndex < m_menus.size());
	assert(itemIndex.itemIndex < m_menus[itemIndex.menuIndex].items.size());
...
MenuBar& MenuBar::setItemChecked(const size_t menuIndex, const size_t itemIndex, const bool checked)
{
	return setItemChecked(MenuBarItemIndex{ menuIndex, itemIndex }, checked);
}

MenuBar& MenuBar::setItemChecked(const MenuBarItemIndex& itemIndex, const bool checked)
{
	assert(itemIndex.menuIndex < m_menus.size());
	assert(itemIndex.itemIndex < m_menus[itemIndex.menuIndex].items.size());
...
bool MenuBar::getItemChecked(const size_t menuIndex, const size_t itemIndex) const
{
	return getItemChecked(MenuBarItemIndex{ menuIndex, itemIndex });
}

bool MenuBar::getItemChecked(const MenuBarItemIndex& itemIndex) const
{
	assert(itemIndex.menuIndex < m_menus.size());
	assert(itemIndex.itemIndex < m_menus[itemIndex.menuIndex].items.size());
...
MenuBar& MenuBar::setColorPalette(const ColorPalette& palette) noexcept
{
	m_palette = palette;
	return *this;
}

void MenuBar::reset() noexcept
{
	m_mouseOverMenu.reset();
	m_openMenu.reset();
	m_mouseOverItem.reset();
...
bool MenuBar::itemBoxMouseOver() const
{
	return m_openMenu && m_menus[*m_openMenu].itemBoxRect.mouseOver();
}
